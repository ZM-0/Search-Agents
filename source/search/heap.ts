...
export class MinimumHeap {
     ...
     */
    private readonly evaluate: (node: Node) => number;
     ...
     */
    constructor(evaluate: (node: Node) => number) {
     ...
     */
    public getMinimum(): Node | undefined {
        this.swap(0, this.size() - 1);
        const value: Node | undefined = this.values.pop();
        this.sink(0);
        return value;
    }
     ...
     */
    private getSmallestChild(parent: number): number | null {
        const firstChild: number = 2 * parent + 1;
        const secondChild: number = firstChild + 1;

        if (this.values.length <= firstChild) return null;
    ...
    private swap(index1: number, index2: number): void {
        const buffer: Node = this.values[index1];
